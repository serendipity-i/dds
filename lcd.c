...
//*p:字符串起始地址
//size:字体大小 16/12
void LCD_ShowString(u8 mode,u8 x,u8 y,const u8 *p, u8 size)
{
	u8 MAX_POSX = 128 - size / 2;
  u8 MAX_POSY = 64 - size;         
  while(*p!='\0')
  {       
      if(x>MAX_POSX){x=0;y+=16;}
      if(y>MAX_POSY){y=x=0;LCD_Clear();}
      LCD_ShowChar(x,y * size + 5,*p,size,mode);	 
      x+=size / 2;
      p++;
  }  
}
...
//size:字体大小 16/12
//count:数字的位数 不足补0
void LCD_ShowNum(u8 mode,u8 x,u8 y,u32 Num, u8 size, u8 count)
{
	u8 Num_Temp[Num_Max], i = 1; 
	u8 MAX_POSX = 128 - size / 2;
  u8 MAX_POSY = 64 - size;
	if(x>MAX_POSX){x=0;y+=16;}
  if(y>MAX_POSY){y=x=0;LCD_Clear();}
  ...
  {       
	...
	while(i)
	{
		LCD_ShowChar(x,y * size + 5,Num_Temp[i],size,mode);	 
    x+=8;
    i--;
	}
...
void LCD_ShowAllString(u8 mode,u8 x,u8 y,const char *p)
{
#define MAX_CHAR_POSX 122
#define MAX_CHAR_POSY 58          
    while(*p!='\0')
...
//}
void OLED_ShowString(u8 x,u8 y,u8 *chr, u8 size, char ML)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{	
	...
	{
		chr[j]-=128;
		LCD_ShowChar(x,y*size + ML,chr[j],size,0);
	}else
	LCD_ShowChar(x,y*size + ML,chr[j],size,1);
			x+=8;
		if(x>120){x=0;y+=2;}
			j++;
	}
